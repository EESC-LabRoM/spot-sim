...
class CuroboMotionPlanner:
    ...
    def __init__(self, robot, device="cuda:0", caller_device: Optional[str] = None):
        if not CUROBO_AVAILABLE:
            raise ImportError("Curobo não disponível!")

        self.robot = robot
        self.device = device  # CuRobo device (must be cuda:0)
        self.caller_device = caller_device  # Environment device (can be cpu, cuda:0, etc.)
        self.use_ik = USE_IK
    ...
    def _load_robot_config(self, with_collision=False):
        ...
        import yaml
        from ..configs.spot_configs.robot_config import arm_prefix
    ...
    def plan_to_pose(self, target_pos, target_quat, current_joint_pos):
        ...
        try:
            # Store input device for conversion back if not explicitly set
            if self.caller_device is None:
                self.caller_device = str(target_pos.device)

            # Passa para o dispositivo correto (CuRobo requires cuda:0)
            ...
            if result.success.any():
                ...
                # Convert solution back to caller's device
                if self.caller_device is not None and self.caller_device != str(solution.device):
                    solution = solution.to(self.caller_device)
